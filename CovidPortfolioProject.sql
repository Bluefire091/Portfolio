...
-- CURRENT COVID INFECTION RATE FOR THE US POPULATION
...
--THE DATA TYPE FOR total_deaths WAS NVARCHAR AND NEEDED TO BE CHANGED TO INT
...
SELECT CD.continent, CD.location, CD.date, CD.population, CV.new_vaccinations, SUM(CONVERT(INT, CV.new_vaccinations)) OVER (Partition BY CD.location ORDER BY CD.location, CD.date)
FROM Portfolio..CovidDeaths CD AS Rolling_Vaccination_Count
JOIN Portfolio..CovidVaccinations CV
	ON CD.location = CV.location
	AND CD.date= CV.date
